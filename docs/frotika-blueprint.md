...
**`suppliers`** — `company_id`, `type` (enum `gas_station`, `workshop`, `parts`, `customer`, `other`), `name`, `document` (null), `phone`, `city`, `state`, `notes`.
...
4. **`vRec` ≠ `vTPrest`.** `vTPrest` é o total da prestação; `vRec` é o valor a receber. Em subcontratação diferem. **A receita do DRE é `vTPrest`.** `vRec` fica registrado para conferência.
...
7. **O CNPJ do emitente pode não ser da empresa.** Se `emit/CNPJ` ≠ `company.cnpj`, a empresa é tomadora, não emitente — o CT-e é **despesa** de frete contratado, não receita. Avisar o usuário e perguntar, não adivinhar. No MVP: bloquear com mensagem clara e registrar como caso a implementar.
