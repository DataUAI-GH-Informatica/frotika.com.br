@php
    ...
    $launchModeVal = old('launch_mode', 'single');
...
<script>
    (function() {
        ...
        function applyLaunchMode(mode) {
            if (!statusForecast || !statusSettled) {
                return;
            }

            var requiresForecast = mode === 'monthly' || mode === 'installment';

            if (requiresForecast) {
                statusForecast.checked = true;
                statusSettled.disabled = true;
        ...
        var referenceField = document.getElementById('reference-date-field');
        ...
        document.querySelectorAll('[data-launch-mode]').forEach(function(radio) {
            radio.addEventListener('change', function() {
                ...
                if (referenceField) {
                    referenceField.classList.toggle('hidden', mode === 'single');
                }

                if (installmentsField) {
                    installmentsField.classList.toggle('hidden', mode !== 'installment');
                }

                if (competenceField) {
                    competenceField.classList.toggle('hidden', mode === 'monthly');
                }
            });
        });

        var selectedLaunchMode = document.querySelector('[data-launch-mode]:checked');

        if (selectedLaunchMode) {
            applyLaunchMode(selectedLaunchMode.value);
        }
    })();
</script>
