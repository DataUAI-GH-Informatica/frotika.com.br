...
@section('content')
    <div class="mb-4 flex items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-0.5 text-sm text-slate-500">{{ $entries->total() }}
                {{ \Illuminate\Support\Str::plural('lançamento', $entries->total()) }}</p>
        </div>
        @if ($canManage)
            <div class="hidden lg:block">
                <x-ui.link-button href="{{ route('financial-entries.create') }}" variant="primary">Novo
                    lançamento</x-ui.link-button>
            </div>
        @endif
    </div>
    ...
    <form method="GET" action="{{ route('financial-entries.index') }}"
        ...
        <select name="account" class="h-9 rounded-md border border-slate-300 bg-white px-2 text-sm text-slate-900">
            ...
            @foreach ($accounts as $account)
                <option value="{{ $account->getKey() }}" @selected($filters['account'] === (int) $account->getKey())>{{ $account->getAttribute('name') }}
                </option>
            @endforeach
        </select>
        ...
        <select name="vehicle" class="h-9 rounded-md border border-slate-300 bg-white px-2 text-sm text-slate-900">
            ...
            @foreach ($vehicles as $vehicle)
                <option value="{{ $vehicle->getKey() }}" @selected($filters['vehicle'] === (int) $vehicle->getKey())>
        ...
        <div class="flex gap-2 lg:col-span-2">
            <input type="date" name="from" value="{{ $filters['from'] }}"
                class="h-9 w-full rounded-md border border-slate-300 bg-white px-2 text-sm text-slate-900" />
        ...
        <select name="category"
            ...
            @foreach ($categories as $category)
                <option value="{{ $category->getKey() }}" @selected($filters['category'] === (int) $category->getKey())>
                    {{ $category->getAttribute('code') }} — {{ $category->getAttribute('name') }}</option>
            @endforeach
        </select>

        <div class="flex gap-2 lg:col-span-2">
            <x-ui.button type="submit" class="w-full">Filtrar</x-ui.button>
            <x-ui.link-button href="{{ route('financial-entries.index') }}" variant="secondary"
                class="w-full justify-center">Limpar</x-ui.link-button>
        </div>
    </form>
            ...
            <table class="w-full text-sm">
                <thead class="sticky top-0 z-10 bg-slate-50">
                    <tr class="border-b border-slate-200">
                        ...
                        <th class="px-3 py-2 text-left text-2xs font-semibold uppercase tracking-wide text-slate-500">
                            Situação</th>
                        <th class="px-3 py-2 text-right text-2xs font-semibold uppercase tracking-wide text-slate-500">Valor
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($entries as $entry)
                        @php
                            ...
                            $signed =
                                $entry->getAttribute('type') === FinancialEntryType::Expense
                                    ? -$amountCents
                                    : $amountCents;
                        ...
                        <tr class="h-9 cursor-pointer border-b border-slate-100 hover:bg-slate-50"
                            ...
                            <td class="px-3 font-mono tabular text-slate-600">
                                {{ Format::date($entry->getAttribute('competence_date')) }}</td>
                            <td class="px-3 font-mono tabular text-slate-600">
                                {{ Format::date($entry->getAttribute('reference_date')) ?: '—' }}</td>
                            <td class="max-w-xs truncate px-3 text-slate-900">{{ $entry->getAttribute('description') }}
                            </td>
                            <td class="px-3 font-mono text-xs tabular text-slate-500">
                                {{ $entry->category?->getAttribute('code') }}</td>
                            <td class="px-3"><span
                                    class="inline-flex items-center rounded-full border px-2 py-0.5 text-2xs font-semibold {{ $statusChip($entry->getAttribute('status')) }}">{{ $entry->getAttribute('status')->label() }}</span>
                            </td>
                            <td @class([
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="px-4 py-12 text-center">
                                    <p class="font-display text-lg font-semibold text-slate-900">Nenhum lançamento
                                        encontrado.</p>
                                    <p class="mx-auto mt-1 max-w-sm text-sm text-slate-500">Importe CT-e para gerar receitas
                                        automaticamente ou registre um lançamento manual.</p>
                                    @if ($canManage)
                                        <div class="mt-4 flex justify-center">
                                            <x-ui.link-button href="{{ route('financial-entries.create') }}"
                                                variant="primary">Novo lançamento</x-ui.link-button>
                                        </div>
                                    @endif
                                </div>
                            </td>
                ...
                @if ($entries->isNotEmpty())
                    <tfoot class="sticky bottom-0 bg-slate-50">
                        <tr class="border-t border-slate-200 text-sm">
                            <td colspan="5"
                                class="px-3 py-2 text-right text-2xs font-semibold uppercase tracking-wide text-slate-500">
                                Receitas {{ Format::money($totals['revenue_cents']) }} · Despesas
                                {{ Format::money(-$totals['expense_cents']) }} · Resultado
                            </td>
                            <td @class([
        ...
        <div class="divide-y divide-slate-100 md:hidden">
            @forelse ($entries as $entry)
                @php
                    $amountCents = (int) $entry->getAttribute('amount_cents');
                    $signed =
                        $entry->getAttribute('type') === FinancialEntryType::Expense ? -$amountCents : $amountCents;
                @endphp
                <a href="{{ route('financial-entries.show', ['entry' => $entry->getKey()]) }}"
                    ...
                    <div class="mt-0.5 flex items-center gap-2 text-xs text-slate-500">
                        ...
                        <span
                            class="inline-flex items-center rounded-full border px-1.5 py-0.5 text-2xs font-semibold {{ $statusChip($entry->getAttribute('status')) }}">{{ $entry->getAttribute('status')->label() }}</span>
                    </div>
                </a>
            @empty
                <div class="px-4 py-12 text-center">
