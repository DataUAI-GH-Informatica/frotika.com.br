...
@section('content')
    <div class="mx-auto max-w-3xl">
        <x-ui.page-header title="Lançamento" :subtitle="$entry->getAttribute('description')">
            <x-slot:actions>
                ...
                @if (
                    $canManage &&
                        !$isSynced &&
                        $entry->getAttribute('transfer_pair_id') === null &&
                        $status !== FinancialEntryStatus::Canceled)
                    <x-ui.link-button href="{{ route('financial-entries.edit', ['entry' => $entry->getKey()]) }}"
        ...
        @if (session('warning'))
            <div class="mb-4 rounded-md border border-warning-300 bg-warning-50 px-4 py-3 text-sm text-warning-700">
                {{ session('warning') }}</div>
        @endif
        ...
        <x-ui.card class="border-slate-200 bg-white">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-2 font-display text-lg font-semibold text-slate-900">
                        {{ $entry->getAttribute('description') }}</p>
                </div>
                <div @class([
            ...
            <dl class="mt-5 grid gap-x-6 gap-y-3 border-t border-slate-200 pt-4 text-sm sm:grid-cols-2">
                <div class="flex justify-between sm:block">
                    ...
                    <dd class="text-slate-900">{{ $entry->category?->getAttribute('code') }} —
                        {{ $entry->category?->getAttribute('name') }}</dd>
                </div>
                <div class="flex justify-between sm:block">
                ...
                <div class="flex justify-between sm:block">
                    ...
                    <dd class="font-mono tabular text-slate-900">{{ Format::date($entry->getAttribute('paid_at')) ?: '—' }}
                    </dd>
                </div>
                <div class="flex justify-between sm:block">
                ...
                @if ($vehicle !== null)
                    <div class="flex justify-between sm:block">
                        ...
                        <dd><a href="{{ route('vehicles.show', ['vehicle' => $vehicle->getKey()]) }}"
                                class="font-mono tabular text-brand-700 hover:text-brand-800">{{ Format::plate($vehicle->getAttribute('plate')) }}</a>
                        </dd>
                    </div>
                @endif
        ...
        @if ($canManage && $status === FinancialEntryStatus::Forecast)
            <x-ui.card class="mt-4 border-slate-200 bg-white">
                ...
                <p class="mt-0.5 text-sm text-slate-500">Marque como
                    {{ $type === FinancialEntryType::Revenue ? 'recebido' : 'pago' }} informando a conta e a data.</p>

                <form method="POST" action="{{ route('financial-entries.settle', ['entry' => $entry->getKey()]) }}"
                    class="mt-4 grid gap-4 sm:grid-cols-3">
                    @csrf
                    <x-ui.select label="Conta bancária" name="bank_account_id" required>
                        <option value="">Selecione…</option>
                        @foreach ($accounts as $account)
                            <option value="{{ $account->getKey() }}" @selected((int) old('bank_account_id') === (int) $account->getKey())>
        ...
        @if ($canManage && $status === FinancialEntryStatus::Settled && $entry->getAttribute('transfer_pair_id') === null)
            <x-ui.card class="mt-4 border-slate-200 bg-white">
                <h2 class="font-display text-base font-semibold text-slate-900">Reverter baixa</h2>
                <p class="mt-0.5 text-sm text-slate-500">Use quando este lançamento voltou a ficar pendente. Ele retorna
                    para previsto e sai do saldo realizado.</p>

                <form method="POST" action="{{ route('financial-entries.unsettle', ['entry' => $entry->getKey()]) }}"
                    class="mt-4" onsubmit="return confirm('Reverter a baixa deste lançamento?');">
                    @csrf
                    <x-ui.button type="submit" variant="secondary">Reverter baixa</x-ui.button>
                </form>
            </x-ui.card>
        @endif
